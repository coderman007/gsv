            ...
            <div class="mt-5 bg-white rounded-lg shadow p-6">
                ...
                <div class="mb-4">
                    ...
                    <div class="flex items-center mt-2">
                        <input type="text" value="{{ $vehicle_type }}" class="text-gray-800 w-full px-4 py-2.5 text-base transition duration-500 ease-in-out border border-gray-300 rounded-lg focus:outline-none focus:ring-blue-500 focus:border-blue-500" readonly>
                        <button wire:click="toggleVehicleTypeDropdown" class="ml-2 px-4 py-2 text-white bg-blue-500 rounded-lg hover:bg-blue-600 focus:outline-none transition duration-200">
                            Modificar
                        </button>
                    </div>
                    ...
                    @if ($showVehicleTypeDropdown)
                        <div class="mt-2">
                            <select wire:model.live="vehicle_type" class="text-gray-800 w-full px-4 py-2.5 text-base transition duration-500 ease-in-out border border-gray-300 rounded-lg focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                                <option value="" hidden>Seleccionar tipo de vehículo</option>
                                <option value="motocicleta" {{ $vehicle_type === 'motocicleta' ? 'selected' : '' }}>Motocicleta</option>
                                <option value="automóvil" {{ $vehicle_type === 'automóvil' ? 'selected' : '' }}>Automóvil</option>
                                <option value="camión" {{ $vehicle_type === 'camión' ? 'selected' : '' }}>Camión</option>
                                <option value="autobús" {{ $vehicle_type === 'autobús' ? 'selected' : '' }}>Autobús</option>
                                <option value="van" {{ $vehicle_type === 'van' ? 'selected' : '' }}>Van</option>
                                <option value="otro" {{ $vehicle_type === 'otro' ? 'selected' : '' }}>Otro</option>
                            </select>
                        </div>
                    @endif
