<div>
    <button wire:click="$set('openCreate', true)"
        ...
        <span class="relative text-gray-500 transition duration-700 group-hover:text-white ease">
            <div class="flex items-center"><i class="fa fa-solid fa-toolbox mr-2 text-xl"></i> Nueva Herramienta</div>
        </span>
    </button>
    ...
    <x-dialog-modal maxWidth="3xl" wire:model="openCreate">
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow-lg p-6">
            <x-slot name="title">
                <h2 class="text-2xl font-semibold text-center text-gray-500 dark:text-white py-6">Crear Herramienta</h2>
            </x-slot>
            ...
            <x-slot name="content">
                <div class="border border-sky-500 shadow-sm shadow-sky-500 m-4 p-10 rounded-lg">
                    <form class="space-y-5 mx-auto">
                            ...
                            <div class="relative">
                                <label class="flex flex-col items-center justify-center h-48 gap-4 p-6 mx-auto bg-white border-2 border-gray-300 border-dashed rounded-lg shadow-md cursor-pointer w-72">
                                    ...
                                    <div class="absolute top-0 h-48 w-72">
                                        @if ($image)
                                            <img class="object-cover w-full h-full rounded-lg mb-4" src="{{ $image->temporaryUrl() }}" alt="Imagen de Herramienta">
                                        @endif
                                    </div>
                                </label>
                        ...
                        <div class="mb-5">
                            <label for="unitPricePerDay" class="block mb-2 text-sm font-medium text-gray-500 dark:text-white">Precio Diario:</label>
                            <input wire:model="unitPricePerDay" type="number" id="unitPricePerDay" step="0.01"
                                   class="bg-sky-50 border border-sky-300 text-gray-500 text-sm rounded-lg focus:ring-sky-500 focus:border-sky-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-sky-500 dark:focus:border-sky-500"
                                   placeholder="Ingrese el precio diario" required />
            ...
            <x-slot name="footer">
                <div class="flex justify-between gap-4 text-lg">
                    ...
                    <button type="submit" wire:click="createTool"
                            wire:loading.attr="disabled"
                            wire:target="createTool"
