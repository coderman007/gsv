<div class="relative inline-block text-center cursor-pointer group">
    ...
    <x-dialog-modal maxWidth="3xl" wire:model="openShow">
        <div class="w-full mx-auto bg-white dark:bg-gray-800 shadow-md p-6 rounded-md">
            <!-- Título del modal -->
            <x-slot name="title">
                <h2 class="font-semibold text-2xl text-center pt-4 text-gray-500 dark:text-gray-300">Detalle de
                    Herramientas</h2>
            </x-slot>
            <!-- Contenido del modal -->
            <x-slot name="content">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                    ...
                    <div class="space-y-2 md:col-span-2">
                        <p id="name"
                           class="text-sky-500 dark:text-sky-200 text-2xl font-bold p-4 dark:bg-sky-600 rounded-lg">{{ ucfirst($tool->name) }}</p>
                    </div>

                    <!-- Costo -->
                    <div class="space-y-2">
                        <label for="unit_price_per_day" class="font-semibold text-gray-700 dark:text-gray-300 text-lg">Precio
                            Unitario por Día:</label>
                        <div class="px-6 py-4 dark:text-lg">
                            <div class="bg-sky-100 p-2 rounded-sm font-semibold text-center text-lg">
                                <span class="">$</span>
                                <span>{{ number_format($tool->unit_price_per_day, 0, ',', '.') }}</span>
                            </div>
                        </div>
                    </div>
