<div class="flex flex-col w-full">
    ...
    <ul class="overflow-y-auto max-h-96">
        @forelse ($tools as $tool)
            <li class="flex items-center py-2 hover:bg-gray-100">
                <input type="checkbox" wire:model.live="selectedToolsEdit" value="{{ $tool->id }}"
                    ...
                    @if (in_array($tool->id, $selectedToolsEdit))
                        <div class="flex items-center space-x-4 pr-4">
                            ...
                            <div class="flex flex-col">
                                <label for="quantityEdit_{{ $tool->id }}"
                                       class="text-sm text-gray-700 mb-1">Cantidad:</label>
                                <input wire:model.live="quantitiesEdit.{{ $tool->id }}"
                                       id="quantityEdit_{{ $tool->id }}" min=1 type="number"
                                       class="w-16 px-2 py-1 border-sky-300 rounded-md focus:outline-none focus:ring-sky-500 focus:border-sky-500">
                            </div>

                            <!-- Días requeridos -->
                            <div class="flex flex-col">
                                <label for="requiredDaysEdit_{{ $tool->id }}"
                                       class="text-sm text-gray-700 mb-1">Días:</label>
                                <input wire:model.live="requiredDaysEdit.{{ $tool->id }}"
                                       id="requiredDaysEdit_{{ $tool->id }}" min=1 type="number"
                                       class="w-16 px-2 py-1 border-sky-300 rounded-md focus:outline-none focus:ring-sky-500 focus:border-sky-500">
                            </div>

                            <!-- Rendimiento -->
                            <div class="flex flex-col">
                                <label for="efficiencyEdit_{{ $tool->id }}"
                                       class="text-sm text-gray-700 mb-1">Rendimiento:</label>
                                <input wire:model.live="efficiencyInputsEdit.{{ $tool->id }}"
                                       id="efficiencyEdit_{{ $tool->id }}" type="text"
                                       class="w-16 px-2 py-1 border-sky-300 rounded-md focus:outline-none focus:ring-sky-500 focus:border-sky-500">
                            </div>

                            <!-- Costo parcial -->
                            <div class="flex flex-col">
                                <label for="partialCostEdit_{{ $tool->id }}"
                                       class="text-sm text-gray-700 mb-1">Costo:</label>
                                <input type="text" readonly
                                       value="{{ isset($partialCostsEdit[$tool->id]) ? number_format($partialCostsEdit[$tool->id], 2) : 0 }}"
                                       id="partialCostEdit_{{ $tool->id }}"
    ...
    <div class="bg-gray-50 p-2 rounded-lg mb-4 flex items-center">
        <label for="totalToolCostEdit" class="block text-lg mr-4 font-medium text-gray-700">Total Herramientas:</label>
        <div class="relative rounded-md shadow-sm flex-1">
            <input type="text" readonly value="{{ number_format($totalToolCostEdit, 2) }}" id="totalToolCostEdit"
        ...
        <div class="ml-4">
            <button wire:click="sendTotalToolCostEdit" type="button"
