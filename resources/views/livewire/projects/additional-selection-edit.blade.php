<div class="flex flex-col w-full">
    ...
    <ul class="overflow-y-auto max-h-96">
        @forelse ($additionals as $additional)
            <li class="flex items-center py-2 hover:bg-gray-100">
                <input type="checkbox" wire:model.live="selectedAdditionalsEdit" value="{{ $additional->id }}"
                    ...
                    @if (in_array($additional->id, $selectedAdditionalsEdit))
                        <div class="flex items-center space-x-4 pr-4">
                            ...
                            <div class="flex flex-col">
                                <label for="quantityEdit_{{ $additional->id }}"
                                       class="text-sm text-gray-700 mb-1">Cantidad:</label>
                                <input wire:model.live="quantitiesEdit.{{ $additional->id }}"
                                       id="quantityEdit_{{ $additional->id }}" min=1 type="number"
                                       class="w-16 px-2 py-1 border-yellow-300 rounded-md focus:outline-none focus:ring-yellow-500 focus:border-yellow-500">
                            </div>

                            <!-- Rendimiento -->
                            <div class="flex flex-col">
                                <label for="efficiencyEdit_{{ $additional->id }}"
                                       class="text-sm text-gray-700 mb-1">Rendimiento:</label>
                                <input wire:model.live="efficiencyInputsEdit.{{ $additional->id }}"
                                       id="efficiencyEdit_{{ $additional->id }}" type="text"
                                       class="w-16 px-2 py-1 border-yellow-300 rounded-md focus:outline-none focus:ring-yellow-500 focus:border-yellow-500">
                            </div>

                            <!-- Costo parcial -->
                            <div class="flex flex-col">
                                <label for="partialCostEdit_{{ $additional->id }}" class="text-sm text-gray-700 mb-1">Costo:</label>
                                <input type="text" readonly
                                       value="{{ isset($partialCostsEdit[$additional->id]) ? number_format($partialCostsEdit[$additional->id], 2) : 0 }}"
                                       id="partialCostEdit_{{ $additional->id }}"
    ...
    <div class="bg-gray-50 p-2 rounded-lg mb-4 flex items-center">
        <label for="totalAdditionalCostEdit" class="block text-lg mr-4 font-medium text-gray-700">Total Adicionales:</label>
        <div class="relative rounded-md shadow-sm flex-1">
            <input type="text" readonly value="{{ number_format($totalAdditionalCostEdit, 2) }}" id="totalAdditionalCostEdit"
        ...
        <div class="ml-4">
            <button wire:click="sendTotalAdditionalCostEdit" type="button"
