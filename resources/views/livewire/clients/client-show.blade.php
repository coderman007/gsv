                ...
                <div class="md:mx-6">
                    ...
                    <div class="text-lg text-start">
                        ...
                        <div class="mb-3">
                            ...
                            <p class="p-2 bg-gray-200 rounded-md">{{ $client->email }}</p>
                        </div>

                        <div class="mb-3">
                            <h1 class="ml-2 text-sm">Consumo Promedio:</h1>
                            <p class="p-2 bg-gray-200 rounded-md">{{ $client->average_energy_consumption }}</p>
                        </div>

                        <div class="mb-3">
                            <h1 class="ml-2 text-sm"> Dimensión Cubierta:</h1>
                            <p class="p-2 bg-gray-200 rounded-md">{{ number_format($client->roof_dimensions_length * $client->roof_dimensions_width, 2, '.', '') }}</p>
                        </div>

                        <div>
                            @if ($client->status == 'Activo')
                            <h1 class="ml-2 text-sm">Estado:</h1>
                            <p class="p-2 text-gray-100 bg-green-500 rounded-md">{{ $client->status }}</p>
                            @else
                            <h1 class="ml-2 text-sm">Estado:</h1>
                            <p class="p-2 text-gray-100 bg-red-400 rounded-md">{{ $client->status }}</p>
                            @endif
                        </div>
                    </div>
                </div>
